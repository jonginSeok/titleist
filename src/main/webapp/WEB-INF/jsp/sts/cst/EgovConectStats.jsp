...
<div id="content">
	...
	<form name="listForm"
		action="<c:url value='/sts/selectConectStats.do'/>" method="post">
		<input type="submit" id="invisible" class="invisible" /> <input
			type="hidden" name="pdKind"
			value='<c:out value="${statsInfo.pdKind}"/>' /> <input type="hidden"
			name="statsKind" value='<c:out value="${statsInfo.statsKind}"/>' /> <input
			type="hidden" name="detailStatsKind" value="" />

		<!-- 검색 필드 박스 시작 -->
		<div id="search_field">
			<div id="search_field_loc">
				<h2>
					<strong>접속 통계 관리</strong>
				</h2>
			</div>
			<fieldset>
				...
				<div class="sf_start">
					<ul id="search_first_ul">
						<li><label for="fDate">기간 : </label> <input type="hidden"
							name="cal_url"
							value="<c:url value='/sym/cmm/EgovNormalCalPopup.do'/>" /> <input
							type="hidden" name="fromDate" value="${statsInfo.fromDate}"
							size="10" /> <input type="hidden" name="toDate"
							value="${statsInfo.toDate}" size="10" /> <input type="text"
							name="fDate" value="" size="10" title="시작일자(새창)" id="fDate" /> <a
							href="#noscript"
							onclick="fn_egov_NormalCalendar(document.listForm, document.listForm.fromDate, document.listForm.fDate,'','<c:url value='/sym/cmm/EgovselectNormalCalendar.do'/>'); return false;"
							style="selector-dummy: expression(this.hideFocus = false);"><img
								src="<c:url value='/images/calendar.gif' />" alt="달력(새창)"></a>
							&nbsp;~&nbsp;<input type="text" name="tDate" value="" size="10"
							title="종료일자(새창)" id="tDate" /> <a href="#noscript"
							onclick="fn_egov_NormalCalendar(document.listForm, document.listForm.toDate, document.listForm.tDate,'','<c:url value='/sym/cmm/EgovselectNormalCalendar.do'/>'); return false;"
							style="selector-dummy: expression(this.hideFocus = false);"><img
								src="<c:url value='/images/calendar.gif' />" alt="달력(새창)"></a>
						</li>
						<li><label for="PD">기간구분 : </label> <select id="PD" name="PD"
							class="select" onChange="javascript:fnChangePdKind();"
							title="기간구분">
								<option value='Y' selected>연도별</option>
								<option value='M'>월별</option>
								<option value='D'>일별</option>
						</select></li>
						<li><label for="STKIND">통계구분 : </label> <select id="STKIND"
							name="STKIND" class="select"
							onChange="javascript:fnChangeStsKind();" title="통계구분">
								<option value='SERVICE' selected>서비스별</option>
								<!-- option value='PRSONAL'>개인별</option-->
						</select> <!-- 접속통계정보만 확인하므로 개인별 검색조건은 주석
					...
					<ul id="search_second_ul">
						<li>
							<div class="buttons" style="float: right;">
								<a href="#LINK" onclick="fnSearch(); return false;"><img
				...
				<table style="width: 660px;"><!--  width="660" cellpadding="8" border="0" -->
					<c:forEach items="${conectStats}" var="resultInfo"
						...
						<tr>
							...
							<td><img src="<c:url value='/images/left_bg.gif'/>"
