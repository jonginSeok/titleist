...
.table_border{ border-spacing: 10; border-collapse: separate; }
.table_line{ border-spacing: 0; border-collapse: collapse; }
</style>
			<!-- 현재위치 네비게이션 시작 -->
			<div id="content">
				<div id="cur_loc">
				...
				<div id="search_field">
					<div id="search_field_loc">
						<h2>
							<strong>개시판템플릿목록</strong>
						</h2>
					</div>
					<form name="frm"
						action="<c:url value='/cop/com/selectTemplateInfs.do'/>"
						method="post">
						<input type="hidden" name="tmplatId" value="" /> <input
						...
						<fieldset>
							...
							<div class="sf_start">
								<ul id="search_first_ul">
									<li><select name="searchCnd" title="검색조건" class="select">
											<option value="0"
												<c:if test="${searchVO.searchCnd == '0'}">selected="selected"</c:if>>템플릿명</option>
											<option value="1"
												<c:if test="${searchVO.searchCnd == '1'}">selected="selected"</c:if>>템플릿구분</option>
									</select></li>
									<li><input name="searchWrd" title="검색어" type="text"
										size="35" value='<c:out value="${searchVO.searchWrd}"/>'
										maxlength="35" onkeypress="press(event);"></li>
								</ul>
				...
				<div class="default_tablestyle">
					<!-- <table summary="번호,게시판명,사용 커뮤니티 명,사용 동호회 명,등록일시,사용여부   목록입니다" cellpadding="0" cellspacing="0"> --><!-- html 4 -->
					<table class="table_line" summary="번호,게시판명,사용 커뮤니티 명,사용 동호회 명,등록일시,사용여부   목록입니다">
						...
						<tbody>
							...
							<c:forEach var="result" items="${resultList}" varStatus="status">
								...
								<tr>
									...
									<td class="listLeft" nowrap="nowrap"><a
										href="<c:url value='/cop/com/selectTemplateInf.do'/>?tmplatId=<c:out value='${result.tmplatId}'/>">
											<c:out value="${result.tmplatNm}" />
									</a></td>
									<td nowrap="nowrap"><c:out
											value="${result.tmplatSeCodeNm}" /></td>
									<td nowrap="nowrap"><c:out value="${result.tmplatCours}" /></td>
									<td nowrap="nowrap"><c:if test="${result.useAt == 'N'}">
											<spring:message code="button.notUsed" />
										</c:if> <c:if test="${result.useAt == 'Y'}">
							...
							<c:if test="${fn:length(resultList) == 0}">
								<tr>
									<td nowrap colspan="5"><spring:message
				...
				<div id="paging_div">
					<ul class="paging_align">
						<ui:pagination paginationInfo="${paginationInfo}" type="image"
							jsFunction="fn_egov_select_tmplatInfo" />
					</ul>
				</div>
				<!-- //페이지 네비게이션 끝 -->
			</div>
