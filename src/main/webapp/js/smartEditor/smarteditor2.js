/**
 * * SmartEditor2 NHN_Library:SE2.3.1.O9858:SmartEditor2.0-OpenSource *
 * 
 * @version 9858
 */
var nSE2Version = 9858;
		...
		try {
			...
			for ( var i = 0, niLen = aTexts.length; i < niLen; i++) {
				...
				for ( var j = sText.length - nOriginLen; j >= 0; j--) {
					...
					if (bWholeWord && (j > 0 && sText.charAt(j - 1).match(/[a-zA-Z가-힣]/))) {
						continue;
					}
			...
			/*
			 * var t2 = new Date(); console.log("OK"); console.log(sOriginalWord); console.log("MC:"+(t1-t0)); console.log("RP:"+(t2-t1));
			 */

			return nMatchCnt;
		} catch (e) {
			/*
			 * console.log("ERROR"); console.log(sOriginalWord); console.log(new Date()-t0);
			 */
			return nMatchCnt;
		}
	}
...
/*
 * // 0 { htTableProperty : { border : "0", cellPadding : "0", cellSpacing : "0" }, htTableStyle : { border : "1px dashed #666666", borderRight : "0",
 * borderBottom : "0" }, aRowStyle : [ { padding : "3px 0 2px 0", border : "1px dashed #666666", borderTop : "0", borderLeft : "0" } ] }, // 1 {
 * htTableProperty : { border : "0", cellPadding : "0", cellSpacing : "0" }, htTableStyle : { border : "1px solid #c7c7c7", borderRight : "0",
 * borderBottom : "0" }, aRowStyle : [ { padding : "3px 0 2px 0", border : "1px solid #c7c7c7", borderTop : "0", borderLeft : "0" } ] }, // 2 {
 * htTableProperty : { border : "0", cellPadding : "0", cellSpacing : "1" }, htTableStyle : { border : "1px solid #c7c7c7" }, aRowStyle : [ { padding :
 * "2px 0 1px 0", border : "1px solid #c7c7c7" } ] }, // 3 { htTableProperty : { border : "0", cellPadding : "0", cellSpacing : "1" }, htTableStyle : {
 * border : "1px double #c7c7c7" }, aRowStyle : [ { padding : "1px 0 0", border : "3px double #c7c7c7" } ] }, // 4 { htTableProperty : { border : "0",
 * cellPadding : "0", cellSpacing : "1" }, htTableStyle : { borderWidth : "2px 1px 1px 2px", borderStyle : "solid", borderColor : "#c7c7c7" },
 * aRowStyle : [ { padding : "2px 0 0", borderWidth : "1px 2px 2px 1px", borderStyle : "solid", borderColor : "#c7c7c7" } ] }, // 5 { htTableProperty : {
 * border : "0", cellPadding : "0", cellSpacing : "1" }, htTableStyle : { borderWidth : "1px 2px 2px 1px", borderStyle : "solid", borderColor :
 * "#c7c7c7" }, aRowStyle : [ { padding : "1px 0 0", borderWidth : "2px 1px 1px 2px", borderStyle : "solid", borderColor : "#c7c7c7" } ] },
 */
// Black theme ======================================================
// 6
{
...
nhn.husky.HuskyCore = jindo.$Class({
	...
	_execMsgStep : function(sMsgStep, sMsg, args) {
		return (this._execMsgStep = this.htOptions.oDebugger ? this._execMsgStepWithDebugger : this._execMsgStepWithoutDebugger).call(this, sMsgStep,
				sMsg, args);
	},
			...
			for (i = 0; i < iNumOfPlugins; i++) {
				if (!aPlugins[i]._husky_bRun) {
					...
					if (typeof aPlugins[i].$LOCAL_BEFORE_FIRST == "function"
							&& this._execHandler(aPlugins[i], "$LOCAL_BEFORE_FIRST", [ sMsgHandler, args ]) === false) {
						continue;
					}
				}
	...
	_execHandler : function(oPlugin, sHandler, args) {
		return (this._execHandler = this.htOptions.oDebugger ? this._execHandlerWithDebugger : this._execHandlerWithoutDebugger).call(this, oPlugin,
				sHandler, args);
	},
...
nhn.W3CDOMRange = jindo.$Class({
	...
	_compareEndPoint : function(oContainerA, iOffsetA, oContainerB, iOffsetB) {
		...
		// Add 0.5 for each contaienrs that has "moved up" since the actual node is wrapped by 1 or more parent nodes and therefore its position is
		// somewhere between idx & idx+1
...
nhn.HuskyRange = jindo.$Class(
		{
			...
			removeStringBookmark : function(sBookmarkID) {
				/*
				 * var oStartMarker = this._document.getElementById(this.HUSKY_BOOMARK_START_ID_PREFIX+sBookmarkID); var oEndMarker =
				 * this._document.getElementById(this.HUSKY_BOOMARK_END_ID_PREFIX+sBookmarkID); if(oStartMarker)
				 * nhn.DOMFix.parentNode(oStartMarker).removeChild(oStartMarker); if(oEndMarker)
			...
			/**
			 * 노드의 취소선/밑줄 정보를 확인한다 관련 BTS [SMARTEDITORSUS-26]
			 * 
			 * @param {Node}
			 *            oNode 취소선/밑줄을 확인할 노드
			 * @param {String}
			 *            sValue textDecoration 정보
			...
			/**
			 * 노드에 취소선/밑줄을 적용한다 관련 BTS [SMARTEDITORSUS-26] [FF] 노드의 Style 에 textDecoration 을 추가한다 [FF 외] U/STRIKE 태그를 추가한다
			 * 
			 * @param {Node}
			 *            oNode 취소선/밑줄을 적용할 노드
			 * @param {String}
			 *            sValue textDecoration 정보
			 * @see nhn.HuskyRange#_checkTextDecoration
			 */
			...
			/**
			 * 인자로 전달받은 노드 상위의 취소선/밑줄 정보를 확인하여 노드에 적용한다 관련 BTS [SMARTEDITORSUS-26]
			 * 
			 * @param {Node}
			 *            oNode 취소선/밑줄을 적용할 노드
			 */
			_checkTextDecoration : function(oNode) {
				...
				var bUnderline = false, bLineThrough = false, sTextDecoration = "", oParentNode = null;
			 ...
			 * 
			 * @param {Object}
			 *            oStyle 적용할 스타일을 가지는 Object (예) 글꼴 색 적용의 경우 { color : "#0075c8" }
			 * @param {Object}
			 *            [oAttribute] 적용할 속성을 가지는 Object (예) 맞춤범 검사의 경우 { _sm2_spchk: "강남콩", class: "se2_check_spell" }
			 * @param {String}
			 *            [sNewSpanMarker] 새로 추가된 SPAN 노드를 나중에 따로 처리해야하는 경우 마킹을 위해 사용하는 문자열
			 * @param {Boolean}
			 *            [bIncludeLI] LI 도 스타일 적용에 포함할 것인지의 여부 [COM-1051] _getStyleParentNodes 메서드 참고하기
			 * @param {Boolean}
			 *            [bCheckTextDecoration] 취소선/밑줄 처리를 적용할 것인지 여부 [SMARTEDITORSUS-26] _setTextDecoration 메서드 참고하기
			 */
			 ...
			 * 
			 * @param {String}
			 *            [sNewSpanMarker] 새로 추가하는 SPAN 노드를 마킹을 위해 사용하는 문자열
			 * @param {Boolean}
			 *            [bIncludeLI] LI 도 스타일 적용에 포함할 것인지의 여부
			...
			/**
			 * 컨테이너 엘리먼트(elContainer)의 모든 자식노드가 노드 배열(waAllNodes)에 속하는지 확인한다 첫 번째 자식 노드와 마지막 자식 노드가 노드 배열에 속하는지를 확인한다
			 * 
			 * @param {Element}
			 *            elContainer 컨테이너 엘리먼트
			 * @param {jindo.$A}
			 *            waAllNodes Node 의 $A 배열
			 * @param {Node}
			 *            [oNode] 성능을 위한 옵션 노드로 컨테이너의 첫 번째 혹은 마지막 자식 노드와 같으면 indexOf 함수 사용을 줄일 수 있음
			 * @return {Array} Style 을 적용할 노드 배열
			 */
			...
			getLineInfo : function(bAfter) {
				...
				if (oSNode != oStartNode || oENode != oEndNode) {
					...
					var iRelativeStartPos = this._compareEndPoint(nhn.DOMFix.parentNode(oStartNode), this._getPosIdx(oStartNode), this.endContainer,
							this.endOffset);
					var iRelativeEndPos = this._compareEndPoint(nhn.DOMFix.parentNode(oEndNode), this._getPosIdx(oEndNode) + 1, this.startContainer,
							this.startOffset);
...
nhn.BrowserSelection = function(win) {
	...
	/*
	 * var oAgentInfo = jindo.$Agent().navigator(); if(oAgentInfo.ie){ nhn.BrowserSelectionImpl_IE.apply(this); }else{
...
nhn.BrowserSelectionImpl_IE = function() {
	...
	this._W3CRange2IERange = function(oW3CRange) {
		if (this.oLastRange.elStartContainer === oW3CRange.startContainer && this.oLastRange.nStartOffset === oW3CRange.startOffset
				&& this.oLastRange.elEndContainer === oW3CRange.endContainer && this.oLastRange.nEndOffset === oW3CRange.endOffset) {
...
/*
 * [ ADD_APP_PROPERTY 주요 오브젝트를 모든 플러그인에서 this.oApp를 통해서 직접 접근 가능 하도록 등록한다. sPropertyName string 등록명 oProperty object 등록시킬 오브젝트
 * ---------------------------------------------------------------------------]
 */
/*
 * [ REGISTER_BROWSER_EVENT 특정 브라우저 이벤트가 발생 했을때 Husky 메시지를 발생 시킨다. obj HTMLElement 브라우저 이벤트를 발생 시킬 HTML 엘리먼트 sEvent string 발생 대기 할 브라우저 이벤트 sMsg
 * string 발생 할 Husky 메시지 aParams array 메시지에 넘길 파라미터 nDelay number 브라우저 이벤트 발생 후 Husky 메시지 발생 사이에 딜레이를 주고 싶을 경우 설정. (1/1000초 단위)
 * ---------------------------------------------------------------------------]
 */
/*
 * [ DISABLE_MESSAGE 특정 메시지를 코어에서 무시하고 라우팅 하지 않도록 비활성화 한다. sMsg string 비활성화 시킬 메시지
 * ---------------------------------------------------------------------------]
 */
/*
 * [ ENABLE_MESSAGE 무시하도록 설정된 메시지를 무시하지 않도록 활성화 한다. sMsg string 활성화 시킬 메시지
 * ---------------------------------------------------------------------------]
 */
/*
 * [ EXEC_ON_READY_FUNCTION oApp.run({fnOnAppReady:fnOnAppReady})와 같이 run 호출 시점에 지정된 함수가 있을 경우 이를 MSG_APP_READY 시점에 실행 시킨다. 코어에서 자동으로 발생시키는 메시지로 직접
 * 발생시키지는 않도록 한다. none ---------------------------------------------------------------------------]
 */
/**
...
nhn.husky.CorePlugin = jindo.$Class({
	...
	_loadFullPlugin : function(aFilenames, sClassName, sMsgName, oThisRef, oArguments, nIdx) {
		jindo.LazyLoading.load(nhn.husky.SE2M_Configuration.LazyLoad.sJsBaseURI + "/" + aFilenames[nIdx], jindo.$Fn(
				function(aFilenames, sClassName, sMsgName, oThisRef, oArguments, nIdx) {
	...
	$ON_MSG_FULL_PLUGIN_LOADED : function(aFilenames, sClassName, sMsgName, oThisRef, oArguments, oRes) {
		...
		// if there were no $LOCAL_BEFORE_FIRST in already-loaded script, set to accept $LOCAL_BEFORE_FIRST next time as the function could be
		// included in the lazy-loaded script.
...
nhn.husky.SE2M_Toolbar = jindo
		.$Class({
			...
			$ON_MSG_APP_READY : function() {
				...
				/*
				 * var aBtns = jindo.$$("BUTTON", this.toolbarArea); for(var i=0; i<aBtns.length; i++){ this.oApp.registerBrowserEvent(aBtns[i],
				 * "focus", "EVENT_TOOLBAR_MOUSEOVER", []); this.oApp.registerBrowserEvent(aBtns[i], "blur", "EVENT_TOOLBAR_MOUSEOUT", []); }
				 */
			...
			$ON_TOGGLE_TOOLBAR_ACTIVE_LAYER : function(elLayer, elBtn, sOpenCmd, aOpenArgs, sCloseCmd, aCloseArgs) {
				this.oApp.exec("TOGGLE_ACTIVE_LAYER", [ elLayer, "MSG_TOOLBAR_LAYER_SHOWN", [ elLayer, elBtn, sOpenCmd, aOpenArgs ], sCloseCmd,
						aCloseArgs ]);
			},
			...
			$ON_EVENT_TOOLBAR_MOUSEDOWN : function(weEvent) {
				...
				// Check if the button pressed is in active status and has a visible layer i.e. the button had been clicked and its layer is open
				// already. (buttons like font styles-bold, underline-got no sub layer -> childNodes.length<=2)
				// -> In this case, do not close here(mousedown). The layer will be closed on "click". If we close the layer here, the click event
				// will open it again because it toggles the visibility.
				while (elTmp) {
					if (elTmp.className && elTmp.className.match(/active/)
							&& (elTmp.childNodes.length > 2 || elTmp.parentNode.className.match(/se2_pair/))) {
						return;
					}
			...
			/*
			 * 텍스트 툴바의 버튼 라운딩 처리 [MOSS] "[UI개발1팀] 스마트에디터 텍스트툴바버튼 라운딩처리가이드" 참고 - 버튼 라운드 처리를 위한 tool_bg 클래스 추가를 위한 엘리먼트 탐색은 _buttonRound 클래스 사용 (AU
			 * 추가클래스)
			 */
			_setRoundedCornerButton : function() {
				...
				for (i = 0, nLiLen = aSingleLi.length; i < nLiLen; i++) {
					...
					if (welLi.hasClass("husky_seditor_ui_text_more") || welLi.hasClass("husky_seditor_ui_photo_attach")
				...
				for (i = 0, nLiLen = aFirstLi.length; i < nLiLen; i++) {
					...
					if (welLi.hasClass("husky_seditor_ui_fontName") || welLi.hasClass("husky_seditor_ui_text_more")
							|| welLi.hasClass("husky_seditor_ui_photo_attach") || welLi.hasClass("husky_seditor_ui_map_attach")
				...
				for (i = 0, nLiLen = aLastLi.length; i < nLiLen; i++) {
					...
					if (welLi.hasClass("husky_seditor_ui_fontSize") || welLi.hasClass("husky_seditor_ui_text_more")
							|| welLi.hasClass("husky_seditor_ui_photo_attach") || welLi.hasClass("husky_seditor_ui_map_attach")
							|| welLi.hasClass("single_child")) {
						continue;
					}
...
nhn.husky.SE2M_EditingModeChanger = jindo.$Class({
	...
	_assignHTMLElements : function(elAppContainer, htConversionMode) {
		...
		if (typeof (htConversionMode) === 'undefined'
				|| (typeof (htConversionMode.bUseModeChanger) === 'undefined' || htConversionMode.bUseModeChanger === true)) {
...
// }
/*
 * [ LOAD_CONTENTS_FIELD 에디터 초기화 시에 넘어온 Contents(DB 저장 값)필드를 읽어 에디터에 설정한다. bDontAddUndo boolean Contents를 설정하면서 UNDO 히스토리는 추가 하지않는다.
 * ---------------------------------------------------------------------------]
 */
/*
 * [ UPDATE_IR_FIELD 에디터의 IR값을 IR필드에 설정 한다. none ---------------------------------------------------------------------------]
 */
/*
 * [ CHANGE_EDITING_MODE 에디터의 편집 모드를 변경한다. sMode string 전환 할 모드명 bNoFocus boolean 모드 전환 후에 에디터에 포커스를 강제로 할당하지 않는다.
 * ---------------------------------------------------------------------------]
 */
/*
 * [ FOCUS 에디터 편집 영역에 포커스를 준다. none ---------------------------------------------------------------------------]
 */
/*
 * [ SET_IR IR값을 에디터에 설정 한다. none ---------------------------------------------------------------------------]
 */
/*
 * [ REGISTER_EDITING_AREA 편집 영역을 플러그인을 등록 시킨다. 원활한 모드 전환과 IR값 공유등를 위해서 초기화 시에 등록이 필요하다. oEditingAreaPlugin object 편집 영역 플러그인 인스턴스
 * ---------------------------------------------------------------------------]
 */
/*
 * [ MSG_EDITING_AREA_RESIZE_STARTED 편집 영역 사이즈 조절이 시작 되었음을 알리는 메시지. none ---------------------------------------------------------------------------]
 */
/*
 * [ RESIZE_EDITING_AREA 편집 영역 사이즈를 설정 한다. 변경 전후에 MSG_EDITIING_AREA_RESIZE_STARTED/MSG_EDITING_AREA_RESIZE_ENED를 발생 시켜 줘야 된다. ipNewWidth number 새 폭
 * ipNewHeight number 새 높이 ---------------------------------------------------------------------------]
 */
/*
 * [ RESIZE_EDITING_AREA_BY 편집 영역 사이즈를 늘리거나 줄인다. 변경 전후에 MSG_EDITIING_AREA_RESIZE_STARTED/MSG_EDITING_AREA_RESIZE_ENED를 발생 시켜 줘야 된다. 변경치를 입력하면 원래 사이즈에서
 * 변경하여 px로 적용하며, width가 %로 설정된 경우에는 폭 변경치가 입력되어도 적용되지 않는다. ipWidthChange number 폭 변경치 ipHeightChange number 높이 변경치
 * ---------------------------------------------------------------------------]
 */
/*
 * [ MSG_EDITING_AREA_RESIZE_ENDED 편집 영역 사이즈 조절이 끝났음을 알리는 메시지. none ---------------------------------------------------------------------------]
 */
/**
...
nhn.husky.SE_EditingAreaVerticalResizer = jindo.$Class({
	...
	/**
	 * [SMARTEDITORSUS-677] [에디터 자동확장 ON인 경우] 입력창 크기 조절 바의 위치를 확인하여 브라우저 하단에 위치한 경우 자동확장을 멈춤
	 */
	...
	_assignHTMLElements : function(elAppContainer, htConversionMode) {
		...
		if (typeof (htConversionMode) === 'undefined' || typeof (htConversionMode.bUseVerticalResizer) === 'undefined'
				|| htConversionMode.bUseVerticalResizer === true) {
...
// {
/**
 * @fileOverview This file contains Husky plugin that takes care of the operations directly related to editing the HTML source code using Textarea
 *               element
...
nhn.husky.SE_EditingArea_TEXT = jindo.$Class({
	...
	_addLineBreaker : function(sContent) {
		...
		var oContent = new StringBuffer(), aContent = sContent.split('\n'), // \n을 기준으로 블럭을 나눈다.
		aContentLng = aContent.length, sTemp = "";
	...
	/**
	 * HTML 태그에 해당하는 글자가 먹히지 않도록 바꿔주기 동작) & 를 &amp; 로, < 를 &lt; 로, > 를 &gt; 로 바꿔준다
	 * 
	 * @param {String}
	 *            sText
	 * @return {String}
	...
	/**
	 * htmlSpecialChars 의 반대 기능의 함수 동작) &amp, &lt, &gt, &nbsp 를 각각 &, <, >, 빈칸으로 바꿔준다
	 * 
	 * @param {String}
	 *            sText
	 * @return {String}
...
/*
 * [ REFRESH_WYSIWYG (FF전용) WYSIWYG 모드를 비활성화 후 다시 활성화 시킨다. FF에서 WYSIWYG 모드가 일부 비활성화 되는 문제용 주의] REFRESH_WYSIWYG후에는 본문의 selection이 깨져서 커서 제일 앞으로 가는 현상이
 * 있음. (stringbookmark로 처리해야함.) none ---------------------------------------------------------------------------]
 */
/*
 * [ ENABLE_WYSIWYG 비활성화된 WYSIWYG 편집 영역을 활성화 시킨다. none ---------------------------------------------------------------------------]
 */
/*
 * [ DISABLE_WYSIWYG WYSIWYG 편집 영역을 비활성화 시킨다. none ---------------------------------------------------------------------------]
 */
/*
 * [ PASTE_HTML HTML을 편집 영역에 삽입한다. sHTML string 삽입할 HTML oPSelection object 붙여 넣기 할 영역, 생략시 현재 커서 위치
 * ---------------------------------------------------------------------------]
 */
/*
 * [ RESTORE_IE_SELECTION (IE전용) 에디터에서 포커스가 나가는 시점에 기억해둔 포커스를 복구한다. none ---------------------------------------------------------------------------]
 */
/**
...
nhn.husky.SE_EditingArea_WYSIWYG = jindo
		.$Class({
			...
			$init : function(iframe) {
				...
				var sIFrameSrc = this.sIFrameSrc, iframe = this.iframe, fHandlerSuccess = jindo.$Fn(this.initIframe, this).bind(), fHandlerFail = jindo
						.$Fn(function() {
			...
			$ON_MSG_APP_READY : function() {
				...
				// mousedown on iframe -> focus goes into the iframe doc -> beforedeactivate is fired -> empty selection is saved by the plugin ->
				// empty selection is recovered in RESTORE_IE_SELECTION
			...
			_setScrollbarWidth : function() {
				var oDocument = this.getDocument(), elScrollDiv = oDocument.createElement("div");
			...
			_getResizeHeight : function() {
				var elBody = this.getDocument().body, welBody, nBodyHeight, aCopyStyle = [ 'width', 'fontFamily', 'fontSize', 'fontWeight',
						'fontStyle', 'lineHeight', 'letterSpacing', 'textTransform', 'wordSpacing' ], oCss, i;
			...
			_setAutoResize : function() {
				var elBody = this.getDocument().body, welBody = jindo.$Element(elBody), nBodyHeight, nContainerHeight, oCurrentStyle, nStyleSize, bExpand = false, oBrowser = this.oApp.oNavigator;
			...
			/**
			 * 스크롤 처리를 위해 편집영역 Body의 사이즈를 확인하고 설정함 편집영역 자동확장 기능이 Off인 경우에 주기적으로 실행됨
			 */
			_setBodyHeight : function() {
				...
				var elBody = this.getDocument().body, welBody = jindo.$Element(elBody), nMarginTopBottom = parseInt(welBody.css("marginTop"), 10)
						+ parseInt(welBody.css("marginBottom"), 10), nContainerOffset = this.oApp.getEditingAreaHeight(), nMinBodyHeight = nContainerOffset
						- nMarginTopBottom, nBodyHeight = welBody.height(), nScrollHeight, nNewBodyHeight;
			...
			$ON_IE_CHECK_EXCEPTION_FOR_SELECTION_PRESERVATION : function() {
				...
				/*
				 * // [SMARTEDITORSUS-978] HuskyRange.js 의 [SMARTEDITORSUS-888] 이슈 수정과 관련 var tmpSelection = this.getDocument().selection, oRange,
				 * elNode; if(tmpSelection.type === "Control"){ oRange = tmpSelection.createRange(); elNode = oRange.item(0); if(elNode &&
				 * elNode.tagName === "IFRAME"){ this._oIERange = null; } }
				 */
			},
			...
			_onIEBeforeDeactivate : function(wev) {
				...
				/*
				 * [SMARTEDITORSUS-978] HuskyRange.js 의 [SMARTEDITORSUS-888] 이슈 수정과 관련 tmpSelection = this.getDocument().selection; tmpRange =
				 * tmpSelection.createRange(); // Control range does not have parentElement if(tmpRange.parentElement && tmpRange.parentElement() &&
				 * tmpRange.parentElement().tagName === "INPUT"){ this._oIERange = this._oPrevIERange; }else{ this._oIERange = tmpRange; }
				 */
			},
			...
			_backspaceTable : function(weEvent) {
				var oSelection = this.oApp.getSelection(), preNode = null;
			...
			$ON_PASTE_HTML : function(sHTML, oPSelection, bNoUndo) {
				var oSelection, oNavigator, sTmpBookmark, oStartContainer, aImgChild, elLastImg, elChild, elNextChild;
			...
			_getVisibleVerticalPosition : function() {
				var oWindow, oDocument, nVisibleHeight, htPos = {
					nTop : 0,
					nBottom : 0
				};
			...
			_scrollIntoView : function(el) {
				var htElementPos = this._getElementVerticalPosition(el), htVisiblePos = this._getVisibleVerticalPosition(), nScroll = 0;
			...
			/**
			 * EVENT_EDITING_AREA_PASTE 의 ON 메시지 핸들러 위지윅 모드에서 에디터 본문의 paste 이벤트에 대한 메시지를 처리한다. paste 시에 내용이 붙여진 본문의 내용을 바로 가져올 수 없어 delay 를 준다.
			 */
			...
			_pageUp : function(we) {
				var nEditorHeight = this._getEditorHeight(), htPos = jindo.$Document(this.oApp.getWYSIWYGDocument()).scrollPosition(), nNewTop;
			...
			_pageDown : function(we) {
				var nEditorHeight = this._getEditorHeight(), htPos = jindo.$Document(this.oApp.getWYSIWYGDocument()).scrollPosition(), nBodyHeight = this
						._getBodyHeight(), nNewTop;
			...
			initIframe : function() {
				try {
					...
					var sCSSBaseURI = (!!nhn.husky.SE2M_Configuration.SE2M_CSSLoader && nhn.husky.SE2M_Configuration.SE2M_CSSLoader.sCSSBaseURI) ? nhn.husky.SE2M_Configuration.SE2M_CSSLoader.sCSSBaseURI
							: "";
					...
					if (sCSSBaseURI) {
						...
						linkNode.href = '/air/wfw/css/smartEditor/smart_editor2_in.css';
			...
			getIR : function() {
				var sContent = this.iframe.contentWindow.document.body.innerHTML, sIR;
			...
			_recordUndo : function(oKeyInfo) {
				/**
				 * 229: Korean/Eng 16: shift 33,34: page up/down 35,36: end/home 37,38,39,40: left, up, right, down 32: space 46: delete 8: bksp
				 */
...
// }
/**
 * @pluginDesc Enter키 입력시에 현재 줄을 P 태그로 감거나 <br>
 *             태그를 삽입한다.
 */
nhn.husky.SE_WYSIWYGEnterKey = jindo
		.$Class({
			...
			$init : function(sLineBreaker) {
				...
				/**
				 * [SMARTEDITORSUS-230] 밑줄+색상변경 후, 엔터치면 스크립트 오류 [SMARTEDITORSUS-180] [IE9] 배경색 적용 후, 엔터키 2회이상 입력시 커서위치가 다음 라인으로 이동하지 않음 오류 현상 : IE9 에서
				 * 엔터 후 생성된 P 태그가 "빈 SPAN 태그만 가지는 경우" P 태그 영역이 보이지 않거나 포커스가 위로 올라가 보임 해결 방법 : 커서 홀더로 IE 이외에서는 <br>
				 * 을 사용 - IE 에서는 렌더링 시 <br>
				 * 부분에서 비정상적인 P 태그가 생성되어 [SMARTEDITORSUS-230] 오류 발생 unescape("%uFEFF") (BOM) 을 추가 - IE9 표준모드에서 [SMARTEDITORSUS-180] 의 문제가 발생함
				 * (unescape("%u2028") (Line separator) 를 사용하면 P 가 보여지나 사이드이펙트가 우려되어 사용하지 않음) IE 브라우저에서 Enter 처리 시, &nbsp; 를 넣어주므로 해당 방식을 그대로 사용하도록
				 * 수정함
				 */
			...
			/**
			 * IR_TO_DB 변환기 처리 Chrome, FireFox인 경우에만 아래와 같은 처리를 합니다. : 저장 시 본문 영역에서 P 아래의 모든 하위 태그 중 가장 마지막 childNode가 BR인 경우를 탐색하여 이를 &nbsp;로 변경해
			 * 줍니다.
			 */
			onIrToDB : function(sHTML) {
				var sContents = sHTML, rxRegEx = /<br(\s[^>]*)?\/?>((?:<\/span>)?<\/p>)/gi, rxRegExWhitespace = /(<p[^>]*>)(?:[\s^>]*)(<\/p>)/gi;
			...
			_addBlankText : function(oSelection) {
				var oNodes = oSelection.getNodes(), i, nLen, oNode, oNodeChild, tmpTextNode;
				...
				for (i = 0, nLen = oNodes.length; i < nLen; i++) {
					...
					if (!oNodeChild
							...
							|| (oNodeChild.nodeType == 1 && oNode.childNodes.length == 1 && (oNodeChild.id
									.indexOf(oSelection.HUSKY_BOOMARK_START_ID_PREFIX) > -1 || oNodeChild.id
									.indexOf(oSelection.HUSKY_BOOMARK_END_ID_PREFIX) > -1))) {
			...
			_wrapBlock : function(oEvent, sWrapperTagName) {
				var oSelection = this.oApp.getSelection(), sBM = oSelection.placeStringBookmark(), oLineInfo = oSelection.getLineInfo(), oStart = oLineInfo.oStart, oEnd = oLineInfo.oEnd, oSWrapper, oEWrapper, elStyleOnlyNode;
				...
				if (this.htBrowser.firefox) {
					...
					var elParentNode = elBookmark.parentNode, bAddUnderline = false, bAddLineThrough = false;
					...
					bAddLineThrough = (elParentNode.tagName === "S" || elParentNode.tagName === "STRIKE" || (nhn.husky.SE2M_Utils
							.findAncestorByTagName("S", elParentNode) !== null && nhn.husky.SE2M_Utils.findAncestorByTagName("STRIKE", elParentNode) !== null));
			...
			_addExtraCursorHolder : function(elUpperNode) {
				var oNodeChild, oPrevChild, elHtml;
				...
				var oSelection = this.oApp.getSelection(), sBM, elLowerNode, elParent;
			...
			_addTextDecorationTag : function(bAddUnderline, bAddLineThrough) {
				var oTargetNode, oNewNode, oSelection = this.oApp.getSelection();
			...
			_addBlankTextAllSpan : function(elNode) {
				var aSpanList, nSpanLen, sInnerHtml, i;
			...
			_getStyleNode : function(elNode) {
				...
				if (elFirstChild.nodeType === 3
						|| (elFirstChild.nodeType === 1 && (elFirstChild.tagName == "IMG" || elFirstChild.tagName == "BR"
								|| elFirstChild.tagName == "HR" || elFirstChild.tagName == "IFRAME"))) {
					return elNode;
				}
...
nhn.ColorPicker = jindo.$Class(
		{
			...
			_paintColWithCanvas : function() {
				var cvs = jindo.$Element("<canvas>").css({
					width : "100%",
					height : "100%"
				});
					...
					jindo.$Element("<div>").css(
							{
								...
								top : sp + "px",
								height : (ep - sp) + "px",
								filter : "progid:DXImageTransform.Microsoft.Gradient(GradientType=0,StartColorStr='" + s_hex + "',EndColorStr='"
										+ e_hex + "')"
							}).appendTo(this.elem);
				}
			...
			_paintHueWithFilter : function() {
				...
				for ( var i = 1; i < 7; i++) {
					...
					elDiv = jindo.$Element("<div>").css(
							{
								position : "absolute",
								filter : "progid:DXImageTransform.Microsoft.Gradient(GradientType=" + (vert ? 0 : 1) + ",StartColorStr='" + s_hex
										+ "',EndColorStr='" + e_hex + "')"
							});
...
nhn.husky.SE2M_Accessibility = jindo.$Class({
	...
	/*
	 * elAppContainer : mandatory sLocale, sEditorType : optional
	 */
	...
	$ON_FOCUS_NEXT_ELEMENT : function() {
		if (!!this.htAccessOption.sNextElementId) {
			...
			if (document.getElementById(sNextElementId)) {
				...
				// [SMARTEDITORSUS-1360]
				/*
				 * if(jindo.$Agent().navigator().ie && jindo.$Agent().navigator().version == 7) { window.focus();
				 * document.getElementById(sNextElementId).focus(); } else { document.getElementById(sNextElementId).focus(); }
				 */
		...
		.$Class(
				{
					...
					_assignHTMLElements : function(oAppContainer) {
						...
						this.elColorPaletteLayerColorPicker = jindo.$$
								.getSingle("DIV.husky_se2m_color_palette_colorpicker", this.elColorPaletteLayer);
					...
					$ON_APPLY_COLOR : function(elButton) {
						var sColorCode = this.elInputColorCode.value, welColorParent = null;
					...
					_redrawRecentColorElement : function() {
						var aRecentColorHtml = [], nRecentColor = this.aRecentColor.length, i;
					...
					_ajaxRecentColorCallback : function(htResponse) {
						var aColorList = htResponse.json()["result"], waColorList, i, nLen;
...
nhn.husky.SE2M_ExecCommand = jindo
		.$Class({
			...
			_removeSpanAlign : function() {
				var oSelection = this.oApp.getSelection(), aNodes = oSelection.getNodes(), elNode = null;
			...
			_fixAlign : function(sAlign) {
				var oSelection = this.oApp.getSelection(), aNodes = [], elNode = null, elParentNode = null;
				...
				for ( var i = 0, nLen = aNodes.length; i < nLen; i++) {
					...
					if (elParentNode && elParentNode.align !== elParentNode.style.textAlign) { // [SMARTEDITORSUS-704] align 속성과 text-align 속성의 값을
						// 맞춰줌
			...
			_fixDocumentBR : function() {
				...
				var waBeforeBRs = jindo.$A(this.aBeforeBRs), i, iLen = this.aBRs.length;
			...
			_indentMargin : function(elDiv) {
				var elTmp = elDiv, aAppend, i, nLen, elInsertTarget, elDeleteTarget, nCurMarginLeft;
			...
			_outdentMargin : function(elDiv) {
				var elTmp = elDiv, elParentNode, elInsertBefore, elNewParent, elInsertParent, oDoc, nCurMarginLeft;
			...
			// When insertorderedlist/insertunorderedlist is executed on a blockquote, the blockquote will "suck in" directly neighboring OL, UL's if
			// there's any.
			...
			_fixCorruptedBlockQuote : function(sTagName) {
				var aNodes = this.oApp.getWYSIWYGDocument().getElementsByTagName(sTagName), elCorruptedBlockQuote, elTmpParent, elNewNode, aLists, i, nLen, nPos, el, oSelection;
			...
			_extendBlock : function() {
				...
				var oSelection = this.oApp.getSelection(), oStartContainer = oSelection.startContainer, oEndContainer = oSelection.endContainer, aChildImg = [], aSelectedImg = [], oSelectionClone = oSelection
						.cloneRange();
...
nhn.husky.SE2M_FontNameWithLayerUI = jindo
		.$Class({
			...
			_onMouseupCover : function(e) {
				...
				var oMouse = e.mouse(), elBody = this.oApp.getWYSIWYGDocument().body, welBody = jindo.$Element(elBody), oSelection = this.oApp
						.getEmptySelection();
			...
			_addFontToMenu : function(sDisplayName, sFontFamily, sSampleText) {
				...
				elItem.innerHTML = this.elFontItemTemplate.innerHTML.replace("@DisplayName@", sDisplayName).replace("FontFamily", sFontFamily)
						.replace("@SampleText@", sSampleText);
			...
			$ON_SE2M_TOGGLE_FONTNAME_LAYER : function() {
				this.oApp.exec("TOGGLE_TOOLBAR_ACTIVE_LAYER", [ this.oDropdownLayer, null, "MSG_FONTNAME_LAYER_OPENED", [],
			...
			_getFontFamilyFromLI : function(elLi) {
				...
				// <li><button type="button"><span>돋음</span>(</span><em
				// style="font-family:'돋음',Dotum,'굴림',Gulim,Helvetica,Sans-serif;">돋음</em><span>)</span></span></button></li>
			...
			addFont : function(fontId, fontName, defaultSize, fontURL, fontCSSURL, fontType, sSampleText) {
				...
				if (!fontType) {
					sFontFamily = nhn.husky.SE2M_FontNameWithLayerUI.CUSTOM_FONT_MARKS + sFontFamily
				...
				/*
				 * // do not add again, if the font is already in the list for(var i=0; i<this._allFontList.length; i++){ if(newFont.fontFamily ==
				 * this._allFontList[i].fontFamily){ return this._allFontList[i]; } } this._allFontList[this._allFontList.length] = newFont;
				 */
...
nhn.husky.SE2M_Hyperlink = jindo.$Class({
	...
	$ON_TOGGLE_HYPERLINK_LAYER : function() {
		...
		this.oApp.delayedExec("TOGGLE_TOOLBAR_ACTIVE_LAYER", [ this.oHyperlinkLayer, null, "MSG_HYPERLINK_LAYER_SHOWN", [],
			...
			if (this.oSelection.collapsed) {
				...
				if (sURL == "") { // unlink
					...
					if (this._isExceptional()) {
						...
						this.oApp.exec("EXECCOMMAND", [ "createLink", false, this.sATagMarker + nSession + encodeURIComponent(sURL), {
							bDontAddUndoHistory : true
						} ]);
					}
				}
	...
	_isExceptional : function() {
		var oNavigator = jindo.$Agent().navigator(), bImg = false, bEmail = false;
	...
	irToDb : function(oTmpNode) {
		...
		/*
		 * // 텍스트 검색이 용이 하도록 끊어진 텍스트 노드가 있으면 합쳐줌. (화면상으로 ABC라고 보이나 상황에 따라 실제 2개의 텍스트 A, BC로 이루어져 있을 수 있음. 이를 ABC 하나의 노드로 만들어 줌.) // 문제 발생 가능성에 비해서
		 * 퍼포먼스나 사이드 이펙트 가능성 높아 일단 주석 var aCleanTextNodes = []; for(var i=0, nLen=aAllTextNodes.length; i<nLen; i++){ if(a[i].nextSibling &&
		 * a[i].nextSibling.nodeType === 3){ a[i].nextSibling.nodeValue += a[i].nodeValue; a[i].parentNode.removeChild(a[i]); }else{
		...
		for ( var i = 0, nLen = aAllTextNodes.length; i < nLen; i++) {
			...
			elTmpDiv.innerHTML = (sTmpStr + elTmpDiv.innerHTML).replace(
					/(&nbsp|\s)?(((?!http:\/\/)www\.(?:(?!\&nbsp;|\s|"|').)+)|(http:\/\/(?:(?!&nbsp;|\s|"|').)+))/ig, this._generateAutoLink);
...
nhn.husky.SE2M_LineHeightWithLayerUI = jindo.$Class({
	...
	$LOCAL_BEFORE_FIRST : function() {
		...
		for ( var i = 0; i < this.aLIOptions.length; i++) {
			this.oApp.registerBrowserEvent(this.aLIOptions[i], "click", "SET_LINEHEIGHT_FROM_LAYER_UI", [ this
					._getLineHeightFromLI(this.aLIOptions[i]) ]);
		}
...
nhn.husky.SE2M_LineStyler = jindo.$Class({
	...
	getTextNodes : function(bSplitTextEndNodes, oSelection) {
		var txtFilter = function(oNode) {
			...
			if ((oNode.nodeType == 3 && oNode.nodeValue != "\n" && oNode.nodeValue != "") || (oNode.tagName == "LI" && oNode.innerHTML == "")
					|| (oNode.tagName == "P" && oNode.innerHTML == "")) {
				return true;
			} else {
				return false;
			}
...
nhn.husky.SE_WYSIWYGStyleGetter = jindo.$Class({
	...
	$ON_EVENT_EDITING_AREA_MOUSEUP : function(oEvnet) {
		/*
		 * if(this.hKeyUp){ clearTimeout(this.hKeyUp); } this.oApp.delayedExec("CHECK_STYLE_CHANGE", [], 100);
		 */
		this.oApp.exec("CHECK_STYLE_CHANGE");
	},
	...
	$ON_EVENT_EDITING_AREA_KEYPRESS : function(oEvent) {
		...
		/*
		 * // queryCommandState often fails to return correct result for Korean/Enter. So just ignore them. if(this.oApp.oNavigator.firefox &&
		 * (oKeyInfo.keyCode == 229 || oKeyInfo.keyCode == 13)){ return; }
		 */
	...
	$ON_EVENT_EDITING_AREA_KEYDOWN : function(oEvent) {
		...
		/*
		 * backspace 8 enter 13 page up 33 page down 34 end 35 home 36 left arrow 37 up arrow 38 right arrow 39 down arrow 40 insert 45 delete 46
		 */
		// other key strokes are taken care by keypress event
		if (!(oKeyInfo.keyCode == 8 || (oKeyInfo.keyCode >= 33 && oKeyInfo.keyCode <= 40) || oKeyInfo.keyCode == 45 || oKeyInfo.keyCode == 46)) {
			return;
		}
...
nhn.husky.SE_WYSIWYGStyler = jindo.$Class({
	...
	_addCursorHolder : function(oSelection, oSpan) {
		var oBody = this.oApp.getWYSIWYGDocument().body, oAncestor, welSpan = jindo.$Element(oSpan), sHtml, tmpTextNode;
		...
		if (oSelection.collapsed && !bSelectedBlock) {
			...
			if (bNewSpan) {
				if (oSelection.startContainer.tagName == "BODY" && oSelection.startOffset === 0) {
					...
					if (bAppendable && elTmp.nodeType == 1 && elTmp.tagName == "BR") {// [SMARTEDITORSUS-311] [FF4] Cursor Holder 인 BR 의 하위노드로 SPAN
						// 을 추가하여 발생하는 문제
						oSelection.selectNode(oVeryFirstNode);
						oSelection.collapseToStart();
						oSelection.insertNode(oSpan);
					} else if (bAppendable && oVeryFirstNode.tagName != "IFRAME" && oVeryFirstNode.appendChild
							&& typeof oVeryFirstNode.innerHTML == "string") {
			...
			/*
			 * var oCursorStyle = this.oApp.getCurrentStyle(); if(oCursorStyle.bold == "@^"){ this.oApp.delayedExec("EXECCOMMAND", ["bold"], 0); }
			 * if(oCursorStyle.underline == "@^"){ this.oApp.delayedExec("EXECCOMMAND", ["underline"], 0); } if(oCursorStyle.italic == "@^"){
			 * this.oApp.delayedExec("EXECCOMMAND", ["italic"], 0); } if(oCursorStyle.lineThrough == "@^"){ this.oApp.delayedExec("EXECCOMMAND",
			 * ["strikethrough"], 0); }
			 */
			// FF3 will actually display %uFEFF when it is followed by a number AND certain font-family is used(like Gulim), so remove the character
			// for FF3
...
nhn.husky.SE2M_Quote = jindo
		.$Class({
			...
			$init : function() {
				...
				this.htQuoteStyles_view["se2_quote2"] = "_zoom:1;margin:0 0 30px 13px;padding:0 8px 0 16px;background:url(" + sImageBaseURL
						+ "/bg_quote2.gif) 0 3px no-repeat;color:#888888;";
				...
				this.htQuoteStyles_view["se2_quote5"] = "_zoom:1;margin:0 0 30px 0;padding:10px;border:1px dashed #cccccc;background:url("
						+ sImageBaseURL + "/bg_b1.png) repeat;_background:none;_filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src='"
						+ sImageBaseURL + "/bg_b1.png',sizingMethod='scale');color:#888888;";
			...
			$ON_REGISTER_CONVERTERS : function() {
				...
				// -> 블로그 개발 쪽에서 처리 하기로 함.
				/*
				 * this.oApp.exec("ADD_CONVERTER", ["DB_TO_IR", jindo.$Fn(function(sContents){ return sContents.replace(/<blockquote[^>]*
				 * class="?vview_quote([0-9]+)"?[^>]*>((?:\s|.)*?)<\/blockquote>/ig, jindo.$Fn(function(m0,sQuoteType,sQuoteContents){ if (/<!--quote_txt-->((?:\s|.)*?)<!--\/quote_txt-->/ig.test(sQuoteContents)){
				 * if(!this.htSE1toSE2Map[sQuoteType]){ return m0; } return '<blockquote
				 * class="se2_quote'+this.htSE1toSE2Map[sQuoteType]+'">'+RegExp.$1+'</blockquote>'; }else{ return ''; } }, this).bind()); },
				 * this).bind()]);
				 */
			},
			...
			/**
			 * 인용구의 중첩 가능한 최대 개수를 넘었는지 확인함 인용구 내부에서 인용구를 적용하면 중첩되지 않으므로 자식노드에 대해서만 확인함
			 */
			...
			_wrapBlock : function(tag, className) {
				var oSelection, oLineInfo, oStart, oEnd, rxDontUseAsWhole = /BODY|TD|LI/i, oStartNode, oEndNode, oNode, elCommonAncestor, elCommonNode, elParentQuote, elInsertBefore, oFormattingNode, elNextNode, elParentNode, aQuoteChild, aQuoteCloneChild, i, nLen, oP, sBookmarkID;
				...
				if (oSelection.startContainer === oSelection.endContainer && oSelection.startContainer.nodeType === 1
			...
			$ON_EVENT_EDITING_AREA_KEYUP : function(weEvent) {
				var oSelection, elParentQuote, oP;
			...
			_isBlankQuote : function(elParentQuote) {
				var elChild, aChildNodes, i, nLen, bChrome = this.oApp.oNavigator.chrome, bSafari = this.oApp.oNavigator.safari, isBlankText = function(
						sText) {
				...
				if (bChrome || bSafari) { // [SMARTEDITORSUS-352], [SMARTEDITORSUS-502]
					var aTable = jindo.$$("TABLE", elParentQuote), nTable = aTable.length, elTable;
...
nhn.husky.SE2M_TableCreator = jindo
		.$Class({
			...
			irToDbDOM : function(oTmpNode) {
				/**
				 * 저장을 위한 Table Tag 는 아래와 같이 변경됩니다. (1) <TABLE> <table border="1" cellpadding="0" cellspacing="0" style="border:1px dashed #c7c7c7;
				 * border-left:0; border-bottom:0;" attr_no_border_tbl="1" class="__se_tbl"> --> <table border="0" cellpadding="1" cellspacing="0"
				 * attr_no_border_tbl="1" class="__se_tbl"> (2)
				 * <TD>
				 * <td style="border:1px dashed #c7c7c7; border-top:0; border-right:0; background-color:#ffef00" width="245">
				 * <p>
				 * &nbsp;
				 * </p>
				 * </td>
			...
			dbToIrDOM : function(oTmpNode) {
				/**
				 * 수정을 위한 Table Tag 는 아래와 같이 변경됩니다. (1) <TABLE> <table border="0" cellpadding="1" cellspacing="0" attr_no_border_tbl="1"
				 * class="__se_tbl"> --> <table border="1" cellpadding="0" cellspacing="0" style="border:1px dashed #c7c7c7; border-left:0;
				 * border-bottom:0;" attr_no_border_tbl="1" class="__se_tbl"> (2)
				 * <TD>
				 * <td style="background-color:#ffef00" width="245">&nbsp;</td>
				 * -->
				 * <td style="border:1px dashed #c7c7c7; border-top:0; border-right:0; background-color:#ffef00" width="245">
			...
			$LOCAL_BEFORE_FIRST : function() {
				...
				this.oApp.exec("TABLE_SET_BORDER_COLOR", [ /#[0-9A-Fa-f]{6}/.test(this.elText_borderColor.value) ? this.elText_borderColor.value
						: "#cccccc" ]);
			...
			_getTableString : function() {
				...
				if (this.nStyleMode == 1) {
					sTable = this._doGetTableString(this.nColumns, this.nRows, this.nBorderSize, this.sBorderColor, this.sBGColor,
			...
			$ON_TABLE_INSERT : function() {
				...
				var sTable, elLinebreak, elBody, welBody, elTmpDiv, elTable, elFirstTD, oSelection, elTableHolder, htBrowser;
				...
				if (htBrowser.ie && this.oApp.getWYSIWYGDocument().body.childNodes.length === 1
						&& this.oApp.getWYSIWYGDocument().body.firstChild === elTableHolder) {
			...
			/**
			 * P 안에 Table 이 추가되지 않도록 P 태그를 분리함 [SMARTEDITORSUS-306] P 에 Table 을 추가한 경우, DOM 에서 비정상적인 P 를 생성하여 깨지는 경우가 발생함 테이블이 추가되는 부분에 P 가 있는 경우, P 를
			 * 분리시켜주는 처리
			 */
			_divideParagraph : function(oSelection) {
				var oParentP, welParentP, sNodeVaule, sBM, oSWrapper, oEWrapper;
			...
			_doGetTableString : function(nColumns, nRows, nBorderSize, sBorderColor, sBGColor, nBorderStyleIdx) {
				...
				var sTableStyle = this.aTableStyleByBorder[nBorderStyleIdx].replace(/#BorderSize#/g, this.nBorderSize).replace(
						/#BorderSize\*([0-9]+)#/g, function(sAll, s1) {
				...
				var sTDStyle = this.aTDStyleByBorder[nBorderStyleIdx].replace(/#BorderSize#/g, this.nBorderSize).replace(/#BorderSize\*([0-9]+)#/g,
						function(sAll, s1) {
							return nBorderSize * parseInt(s1, 10);
						}).replace(/#BorderSize\+([0-9]+)#/g, function(sAll, s1) {
					return nBorderSize + parseInt(s1, 10);
...
nhn.husky.SE2M_TableEditor = jindo
		.$Class({
			...
			STATUS : {
				...
				MOUSEDOWN_BORDER : 6
			// mouse down on the cell resizing grip (cell resizing is in progress)
			},
			...
			$LOCAL_BEFORE_FIRST : function(sMsg) {
				...
				// this.nPageLeftRightMargin = parseInt(elBody.css("marginLeft"), 10)+parseInt(elBody.css("marginRight"), 10) +
				// parseInt(elBody.css("paddingLeft"), 10)+parseInt(elBody.css("paddingRight"), 10);
				// this.nPageTopBottomMargin = parseInt(elBody.css("marginTop"), 10)+parseInt(elBody.css("marginBottom"), 10) +
				// parseInt(elBody.css("paddingTop"), 10)+parseInt(elBody.css("paddingBottom"), 10);
				...
				/*
				 * jindo.$Fn(this._mousedown_WYSIWYGDoc, this).attach(elTarget, "mousedown"); jindo.$Fn(this._mousemove_WYSIWYGDoc,
				 * this).attach(elTarget, "mousemove"); jindo.$Fn(this._mouseup_WYSIWYGDoc, this).attach(elTarget, "mouseup");
				 */
			...
			$ON_TABLE_QE_SET_IMG : function(elSelected) {
				...
				if (n === "0") {
					...
					for ( var j = 0, nLen = this.aSelectedCells.length; j < nLen; j++) {
						jindo.$Element(this.aSelectedCells[j]).css("backgroundImage",
								"url(" + "http://static.se2.naver.com/static/img/" + sImageName + ")");
			...
			$ON_TE_SPLIT_ROW : function() {
				...
				var aTR = jindo.$$(">TBODY>TR", this.elSelectionStartTable, {
					oneTimeOffCache : true
				});
				this._removeClassFromSelection();
			...
			_reassignCellSizes : function(elTable) {
				...
				allCells[0] = jindo.$$(">TBODY>TR>TD", elTable, {
					oneTimeOffCache : true
				});
				allCells[1] = jindo.$$(">TBODY>TR>TH", elTable, {
					oneTimeOffCache : true
				});
				...
				// this is done this way because if the table/cell size were set in %, setting one cell size would change size of other cells, which
				// are still yet in %.
			...
			_isOnBorder : function(wevE) {
				...
				if (!jindo.$Element(this.htResizing.elTable).hasClass(this._sSETblClass)
						&& !jindo.$Element(this.htResizing.elTable).hasClass(this._sSEReviewTblClass)) {
					return;
				}

				// Adjustment variables: to be used to map the x, y position of the resizing point relative to elCell
				// eg) When left border of a cell at 2,2 is selected, the actual cell that has to be resized is the one at 1,2. So, set the horizontal
				// adjustment to -1.
				// Vertical Adjustment
			...
			_showCellResizeGrip : function() {
				...
				this._showResizer();
				if (this.htResizing.nResizeMode == 1) {
					this._setResizerSize((this.htResizing.nBorderSize + this.nDraggableCellEdge) * 2, this.parseIntOr0(jindo.$Element(this.elIFrame)
							.css("height")));
					jindo.$Element(this.elResizeGrid).offset(
							this.htFrameOffset.top,
							this.htFrameOffset.left + this.htResizing.htEPos.clientX - this.parseIntOr0(this.elResizeGrid.style.width) / 2
									- this.htResizing.htEPos.offsetX + this.htResizing.nBorderLeftPos);
				} else {
					...
					jindo.$Element(this.elResizeGrid).offset(
							this.htFrameOffset.top + this.htResizing.htEPos.clientY - this.parseIntOr0(this.elResizeGrid.style.height) / 2
									- this.htResizing.htEPos.offsetY + this.htResizing.nBorderTopPos, this.htFrameOffset.left);
				}
			},
			...
			_insertColumnAfter : function() {
				...
				for ( var y = 0, nYLen = this.htMap[0].length; y < nYLen; y++) {
					...
					for ( var x = this.htSelectionEPos.x; x >= this.htSelectionSPos.x; x--) {
						...
						// elCellClone의 outerHTML에 정상적인 rowSpan이 있더라도 IE에서는 이 위치에서 항상 1을 반환. (elCellClone.rowSpan &
						// elCellClone.getAttribute("rowSpan")).
			...
			_startCellSelection : function() {
				...
				/*
				 * if(!this.oNavigatorInfo["firefox"]){ this.oApp.exec("DISABLE_WYSIWYG", []); }
				 */
			...
			_getCellMapping : function(elTable) {
				var aTR = jindo.$$(">TBODY>TR", elTable, {
					oneTimeOffCache : true
				});
				var nTD = 0;
				var aTD_FirstRow = aTR[0].childNodes;
				/*
				 * // remove empty TR's from the bottom of the table for(var i=aTR.length-1; i>0; i--){ if(!aTR[i].childNodes ||
				 * aTR[i].childNodes.length === 0){ aTR[i].parentNode.removeChild(aTR[i]); aTR = aTR.slice(0, i); if(this.htSelectionSPos.y>=i)
				 * this.htSelectionSPos.y--; if(this.htSelectionEPos.y>=i) this.htSelectionEPos.y--; }else{ break; } }
				 */
				...
				for ( var y = 0; y < nTblY; y++) {
					...
					while (elCell) {
						...
						/*
						 * if(y+rowSpan >= nTblY){ rowSpan = nTblY-y; elCell.setAttribute("rowSpan", rowSpan); }
						 */
			...
			_applyTableTemplate : function(elTable, nTemplateIdx) {
				...
				/*
				 * if(elTable.getAttribute(this.ATTR_TBL_TEMPLATE)){ this._doApplyTableTemplate(elTable,
				 * nhn.husky.SE2M_TableTemplate[this.parseIntOr0(elTable.getAttribute(this.ATTR_TBL_TEMPLATE))], true); }else{
			...
			_clearAllTableStyles : function(elTable) {
				...
				var aTD = jindo.$$(">TBODY>TR>TD", elTable, {
					oneTimeOffCache : true
				});
			...
			_hideTableTemplate : function(elTable) {
				if (elTable.getAttribute(this.ATTR_TBL_TEMPLATE)) {
					this._doApplyTableTemplate(elTable, nhn.husky.SE2M_TableTemplate[this.parseIntOr0(elTable.getAttribute(this.ATTR_TBL_TEMPLATE))],
							true);
				}
			},
			...
			parseBorder : function(vBorder, sBorderStyle) {
				...
				if (isNaN(num)) {
					if (typeof (vBorder) == "string") {
						...
						return 1;
						/*
						 * switch(vBorder){ case "thin": return 1; case "medium": return 3; case "thick": return 5; }
						 */
					}
				}
				return num;
...
nhn.husky.SE2M_QuickEditor_Common = jindo
		.$Class({
			...
			$ON_MSG_APP_READY : function() {
				...
				if (htConfiguration) {
					this._bUseConfig = (!!htConfiguration.common && typeof htConfiguration.common.bUseConfig !== "undefined") ? htConfiguration.common.bUseConfig
							: true;
				}
			...
			/**
			 * 위지윅 영역에 단축키가 등록될 때, tab 과 shift+tab (들여쓰기 / 내어쓰기 ) 를 제외한 단축키 리스트를 저장한다.
			 */
			...
			$ON_MSG_BEFOREUNLOAD_FIRED : function() {
				...
				jindo.$Ajax(this._sAddTextAjaxUrl, {
					...
					onload : function() {
						...
						{
							text_key : "qeditor_fold",
							text_data : "{table:'" + this._environmentData["table"]["type"] + "',img:'" + this._environmentData["img"]["type"]
									+ "',review:'" + this._environmentData["review"]["type"] + "'}"
						});
			},
			/**
			 * openType을 저장하는 함수.
			...
			/**
			 * 레이어가 오픈 할 때 실행되는 이벤트. 레이어가 처음 뜰 때, 저장된 단축키 리스트를 레이어에 등록하고 (레이어가 떠 있을때도 단축키가 먹도록 하기 위해) 레이어에 대한 키보드/마우스 이벤트를 등록한다.
			 * 
			 * @param {Element}
			 *            oEle
			 * @param {Element}
			...
			/*
			 * $ON_EVENT_EDITING_AREA_KEYUP:function(weEvent){
			 * if(this._currentType&&(!this._in_event)&&this._environmentData[this._currentType].isOpen){ this.oApp.exec("CLOSE_QE_LAYER", [weEvent]); }
			 * this._in_event = false; },
			 */
			...
			/**
			 * 늦게 이벤트 바인딩 하는 함수. 레이어가 처음 뜰 때 이벤트를 등록한다.
			 */
			...
			/**
			 * 레이어의 위치를 반환 하는 함수 고정 상태가 아니거나 최소화 상태이면 엘리먼트 위치에 퀵에디터를 띄우고 고정 상태이고 최대화 상태이면 표나 글 양식은 저장된 위치에 띄워주고, 이미지는...?
			 * 
			 * @param {Element}
			 *            oEle
			 * @param {Element}
			...
			calculateLayer : function(oEle, oLayer) {
				/*
				 * 기준을 한군데로 만들어야 함. 1. 에디터는 페이지 2. 엘리먼트는 안에 에디팅 영역 3. 레이어는 에디팅 영역 기준은 페이지로 함.
				 */
			...
			/**
			 * 기준 엘리먼트의 위치를 반환하는 함수 엘리먼트가 있는 경우
			 * 
			 * @param {Element}
			 *            eEle
			 */
...
nhn.DraggableLayer = jindo.$Class({
	...
	_correctXY : function(htXY) {
		...
		if (nTop < this.oOptions.nMinY) {
			nTop = this.oOptions.nMinY;
		}
		if (nTop > this.oOptions.nMaxY) {
			nTop = this.oOptions.nMaxY;
		}

		if (nLeft < this.oOptions.nMinX) {
			nLeft = this.oOptions.nMinX;
		}
		if (nLeft > this.oOptions.nMaxX) {
			nLeft = this.oOptions.nMaxX;
		}
	...
	findNonStatic : function(oEl) {
		...
		if (jindo.$Element(oEl).css("position").match(/absolute|relative/i)) {
			return oEl;
		}
...
/*
 * [ TOGGLE_ACTIVE_LAYER 액티브 레이어가 화면에 보이는 여부를 토글 한다. oLayer HTMLElement 레이어로 사용할 HTML Element sOnOpenCmd string 화면에 보이는 경우 발생 할 메시지(옵션) aOnOpenParam
 * array sOnOpenCmd와 함께 넘겨줄 파라미터(옵션) sOnCloseCmd string 해당 레이어가 화면에서 숨겨질 때 발생 할 메시지(옵션) aOnCloseParam array sOnCloseCmd와 함께 넘겨줄 파라미터(옵션)
 * ---------------------------------------------------------------------------]
 */
/*
 * [ SHOW_ACTIVE_LAYER 액티브 레이어가 화면에 보이는 여부를 토글 한다. oLayer HTMLElement 레이어로 사용할 HTML Element sOnCloseCmd string 해당 레이어가 화면에서 숨겨질 때 발생 할 메시지(옵션)
 * aOnCloseParam array sOnCloseCmd와 함께 넘겨줄 파라미터(옵션) ---------------------------------------------------------------------------]
 */
/*
 * [ HIDE_ACTIVE_LAYER 현재 화면에 보이는 액티브 레이어를 화면에서 숨긴다. none ---------------------------------------------------------------------------]
 */
/**
 ...
 */
/*
 * [ HIDE_LAST_DIALOG_LAYER 마지막으로 화면에 표시한 다이얼로그 레이어를 숨긴다. none ---------------------------------------------------------------------------]
 */
/*
 * [ HIDE_ALL_DIALOG_LAYER 표시 중인 모든 다이얼로그 레이어를 숨긴다. none ---------------------------------------------------------------------------]
 */
/**
 ...
 *              <dd><a
 *              href="http://ajaxui.nhndesign.com/svnview/SmartEditor2_Official/tags/SE2M_popupManager/0.1/test/spec/hp_popupManager_spec.html"
 *              target="_new">Spec</a></dd>
 ...
 * @public
 * @param {Object}
 *            oApp 허스키 코어 객체를 넘겨준다.(this.oApp)
 * @return {Object} nhn.husky.PopUpManager Instance
 * @example 팝업관련 플러그인 제작 예제 nhn.husky.NewPlugin = function(){ this.$ON_APP_READY = function(){ // 팝업 매니저 getInstance 메소드를 호출한다. // 허스키 코어의 참조값을
 *          넘겨준다(this.oApp) this.oPopupMgr = nhn.husky.PopUpMaganer.getInstance(this.oApp); }; // 팝업을 요청하는 메시지 메소드는 아래와 같음
 *          this.$ON_NEWPLUGIN_OPEN_WINDOW = function(){ var oWinOp = { oApp : this.oApp, // oApp this.oApp 허스키 참조값 sUrl : "", // sUrl : 페이지 URL sName :
 *          "", // sName : 페이지 name nWidth : 400, nHeight : 400, bScroll : true } this.oPopUpMgr.openWindow(oWinOp); }; // 팝업페이지 응답데이타 반환 메시지 메소드를
 *          정의함. // 각 플러그인 팝업페이지에서 해당 메시지와 데이타를 넘기게 됨. this.@ON_NEWPLUGIN_WINDOW_CALLBACK = function(){ // 팝업페이지별로 정의된 형태의 아규먼트 데이타가 넘어오면 처리한다. } }
 * @example 팝업 페이지와 opener 호출 인터페이스 예제 onclick시 "nhn.husky.PopUpManager.setCallback(window, "NEWPLUGIN_WINDOW_CALLBACK", oData);" 형태로 호출함.
 */
	...
	if (this._instance == null) {
		...
		this._instance = new (function() {
			...
			this.openWindow = function(oWinOpt) {
				...
				var sProperties = op.sProperties != null ? op.sProperties : "top=" + top + ",left=" + left + ",width=" + op.nWidth + ",height="
						+ op.nHeight + ",scrollbars=" + (op.bScroll ? "yes" : "no") + ",status=yes";
...
nhn.husky.SE2M_UtilPlugin = jindo.$Class({
	...
	$ON_SE2_ATTACH_HOVER_EVENTS : function(aElms) {
		this.oApp.exec("ATTACH_HOVER_EVENTS", [ aElms, {
...
nhn.husky.SE2M_Utils = {
	 ...
	 *            sStr 추출 String
	 * @param {rx}
	 *            rxValue rx type 형식의 값
	 * @param {String}
	 *            sDivision value의 split 형식
	 * @return {Array}
	 */
	 ...
	 *            sKey
	 * @author senxation
	 * @example a = [{ b : "1" }, { b : "2" }] toStringSamePropertiesOfArray(a, "b", ", "); ==> "1, 2"
	 */
	...
	/**
	 * 말줄임을 할때 줄일 내용과 컨테이너가 다를 경우 처리 컨테이너의 css white-space값이 "normal"이어야한다. (컨테이너보다 텍스트가 길면 여러행으로 표현되는 상태)
	 * 
	 * @param {HTMLElement}
	 *            elText 말줄임할 엘리먼트
	 * @param {HTMLElement}
	 *            elContainer 말줄임할 엘리먼트를 감싸는 컨테이너
	 * @param {String}
	 *            sStringTail 말줄임을 표현할 문자열 (미지정시 ...)
	 * @param {Number}
	 *            nLine 최대 라인수 (미지정시 1)
	 * @author senxation
	 * @example //div가 2줄 이하가 되도록 strong 내부의 내용을 줄임 <div> <strong id="a">말줄임을적용할내용말줄임을적용할내용말줄임을적용할내용</strong><span>상세보기</span> <div>
	...
	/**
	 * 최대 가로사이즈를 지정하여 말줄임한다. elText의 css white-space값이 "nowrap"이어야한다. (컨테이너보다 텍스트가 길면 행변환되지않고 가로로 길게 표현되는 상태)
	 * 
	 * @param {HTMLElement}
	 *            elText 말줄임할 엘리먼트
	 * @param {String}
	 *            sStringTail 말줄임을 표현할 문자열 (미지정시 ...)
	 * @param {Function}
	 *            fCondition 조건 함수. 내부에서 true를 리턴하는 동안에만 말줄임을 진행한다.
	 * @author senxation
	 * @example //150픽셀 이하가 되도록 strong 내부의 내용을 줄임 <strong id="a">말줄임을적용할내용말줄임을적용할내용말줄임을적용할내용</strong>> ellipsisByPixel(jindo.$("a"), "...", 150);
	 */
	...
	/**
	 * 여러개의 엘리먼트를 각각의 지정된 최대너비로 말줄임한다. 말줄임할 엘리먼트의 css white-space값이 "nowrap"이어야한다. (컨테이너보다 텍스트가 길면 행변환되지않고 가로로 길게 표현되는 상태)
	 * 
	 * @param {Array}
	 *            aElement 말줄임할 엘리먼트의 배열. 지정된 순서대로 말줄임한다.
	 * @param {String}
	 *            sStringTail 말줄임을 표현할 문자열 (미지정시 ...)
	 * @param {Array}
	 *            aMinWidth 말줄임할 너비의 배열.
	 * @param {Function}
	 *            fCondition 조건 함수. 내부에서 true를 리턴하는 동안에만 말줄임을 진행한다.
	 * @example //#a #b #c의 너비를 각각 100, 50, 50픽셀로 줄임 (div#parent 가 200픽셀 이하이면 중단) //#c의 너비를 줄이는 동안 fCondition에서 false를 리턴하면 b, a는 말줄임 되지 않는다. <div
	 *          id="parent"> <strong id="a">말줄임을적용할내용</strong> <strong id="b">말줄임을적용할내용</strong> <strong id="c">말줄임을적용할내용</strong> <div>
	 *          ellipsisElementsToDesinatedWidth([jindo.$("c"), jindo.$("b"), jindo.$("a")], "...", [100, 50, 50], function(){ if
	 *          (jindo.$Element("parent").width() > 200) { return true; } return false; });
	 */
	 ...
	 * @param {String}
	 *            sTail
	 * @example cutStringToByte('일이삼사오육', 6, '...') ==> '일이삼...' (string)
	 */
	 ...
	 * @return {HashTable}
	 * @author senxation
	 * @example getFilteredHashTable({ a : 1, b : 2, c : 3, d : 4 }, ["a", "c"]); ==> { a : 1, c : 3 }
	 */
	...
	/**
	 * @param {Object}
	 *            oSrc value copy할 object
	 * @return {Object}
	 * @example var oSource = [1, 3, 4, { a:1, b:2, c: { a:1 }}]; var oTarget = oSource; // call by reference oTarget =
	 *          nhn.husky.SE2M_Utils.clone(oSource); oTarget[1] = 2; oTarget[3].a = 100; console.log(oSource); // check for deep copy
	...
	/**
	 * iframe 영역의 aling 정보를 다시 세팅하는 부분. iframe 형태의 산출물을 에디터에 삽입 이후에 에디터 정렬기능을 추가 하였을때 ir_to_db 이전 시점에서 div태그에 정렬을 넣어주는 로직임. 브라우저 형태에 따라 정렬 태그가 iframe을
	 * 감싸는 div 혹은 p 태그에 정렬이 추가된다.
	 * 
	 * @param {HTMLElement}
	 *            el iframe의 parentNode
	 * @param {Document}
	 *            oDoc document
	 */
	...
	/**
	 * jindo.$JSON.fromXML을 변환한 메서드. 소숫점이 있는 경우의 처리 시에 숫자로 변환하지 않도록 함(parseFloat 사용 안하도록 수정) 관련 BTS : [COM-1093]
	 * 
	 * @param {String}
	 *            sXML XML 형태의 문자열
	 * @return {jindo.$JSON}
...
/**
 * nhn.husky.AutoResizer HTML모드와 TEXT 모드의 편집 영역인 TEXTAREA에 대한 자동확장 처리
 */
nhn.husky.AutoResizer = jindo
		.$Class({
			welHiddenDiv : null,
			welCloneDiv : null,
			elContainer : null,
			$init : function(el, htOption) {
				var aCopyStyle = [ 'lineHeight', 'textDecoration', 'letterSpacing', 'fontSize', 'fontFamily', 'fontStyle', 'fontWeight',
						'textTransform', 'textAlign', 'direction', 'wordSpacing', 'fontSizeAdjust', 'paddingTop', 'paddingLeft', 'paddingBottom',
						'paddingRight', 'width' ], i = aCopyStyle.length, oCss = {
					"position" : "absolute",
					"top" : -9999,
					"left" : -9999,
					"opacity" : 0,
			...
			_resize : function() {
				var sContents = this.welTextArea.$value().value, bExpand = false, nHeight;
 ...
 * 
 * @author nox
 * @example var sTmp1 = new StringBuffer(); sTmp1.append('1').append('2').append('3'); var sTmp2 = new StringBuffer('1');
 *          sTmp2.append('2').append('3'); var sTmp3 = new StringBuffer('1').append('2').append('3');
 */
...
// {
/**
 * @fileOverview This file contains Husky plugin that takes care of the operations related to string conversion. Ususally used to convert the IR
 *               value.
...
// }
/*
 * [ ATTACH_HOVER_EVENTS 주어진 HTML엘리먼트에 Hover 이벤트 발생시 특정 클래스가 할당 되도록 설정 aElms array Hover 이벤트를 걸 HTML Element 목록 sHoverClass string Hover 시에 할당 할 클래스
 * ---------------------------------------------------------------------------]
 */
/**
...
/*
 * [ SE_FIT_IFRAME 스마트에디터 사이즈에 맞게 iframe사이즈를 조절한다. none ---------------------------------------------------------------------------]
 */
/**
...
nhn.husky.SE2B_CSSLoader = jindo.$Class({
	...
	aDelayedLoadTrigger : [ "MSG_SE_OBJECT_EDIT_REQUESTED", "OBJECT_MODIFY", "MSG_SE_DUMMY_OBJECT_EDIT_REQUESTED", "TOGGLE_TOOLBAR_ACTIVE_LAYER",
	...
	/*
	 * $BEFORE_REEDIT_ITEM_ACTION : function(){ return this.loadSE2CSS("REEDIT_ITEM_ACTION", arguments); }, $BEFORE_OBJECT_MODIFY : function(){ return
	 * this.loadSE2CSS("OBJECT_MODIFY", arguments); }, $BEFORE_MSG_SE_DUMMY_OBJECT_EDIT_REQUESTED : function(){ return
	 * this.loadSE2CSS("MSG_SE_DUMMY_OBJECT_EDIT_REQUESTED", arguments); }, $BEFORE_TOGGLE_DBATTACHMENT_LAYER : function(){ return
	 * this.loadSE2CSS("TOGGLE_DBATTACHMENT_LAYER", arguments); }, $BEFORE_SHOW_WRITE_REVIEW_DESIGN_SELECT_LAYER : function(){ this.loadSE2CSS(); },
	 * $BEFORE_OPEN_QE_LAYER : function(){ this.loadSE2CSS(); }, $BEFORE_TOGGLE_TOOLBAR_ACTIVE_LAYER : function(){ return
	 * this.loadSE2CSS("TOGGLE_TOOLBAR_ACTIVE_LAYER", arguments); }, $BEFORE_SHOW_TOOLBAR_ACTIVE_LAYER : function(){ return
	 * this.loadSE2CSS("SHOW_TOOLBAR_ACTIVE_LAYER", arguments); }, $BEFORE_SHOW_ACTIVE_LAYER : function(){ this.loadSE2CSS(); },
	 * $BEFORE_SHOW_DIALOG_LAYER : function(){ this.loadSE2CSS(); }, $BEFORE_TOGGLE_ITEM_LAYER : function(){ return
	 * this.loadSE2CSS("TOGGLE_ITEM_LAYER", arguments); },
	 */
	...
	loadSE2CSS : function(sMsg, oArgs) {
		...
		nhn.husky.SE2M_Utils.loadCSS("/air/wfw/css/smartEditor/smart_editor2_items.css", fnCallback);
...
nhn.husky.SE2B_Customize_ToolBar = jindo.$Class(/** @lends nhn.husky.SE2B_Customize_ToolBar */
{
	 ...
	 * @param {Object}
	 *            oAppContainer 툴바 포함 에디터를 감싸고 있는 div 엘리먼트
	 */
	_assignHTMLElements : function(oAppContainer) {
	...
	/**
	 * CHANGE_EDITING_MODE모드 이후에 호출되어야 함. WYSIWYG 모드가 활성화되기 전에 호출이 되면 APPLY_FONTCOLOR에서 에러 발생.
	 */
 ...
 * @name husky_SE2B_Lang_ko_KR.js @ unescape
 */
var oMessageMap = {
